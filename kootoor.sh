...
echo -e "$red▶Coded by$grn KOTTOOR$red◀$rset"
echo -e "$red ⫸$ylo  Devil Psycho$red ⫷$rset"
echo 
# Functions
echo -e "$red [tool is only for educational purposes]$rset"
...
echo -e "$red▶ CREATED by$grn DEVIL PSYCHO$red◀$rset"
echo -e "$red⫸$ylo  KOTTOOR CYBER TEAM$red ⫷$rset"
echo 
# Functions
...
echo -e "$red ➡$cyan [\e[92m1\e[96m] create now"
echo -e "$red ➡$cyan [\e[92m2\e[96m] save the file"
...
then
am start -a android.intent.action.VIEW -d https://youtube.com/channel/UCqvZoq8YHX2bZ94mX_zcSkw> /dev/null 2>&1
banner
menu
elif [ $optnz = "6" ];
then
am start -a android.intent.action.VIEW -d https://github.com/KOTTOORTHARAVADU > /dev/null 2>&1
banner
menu
elif [ $optnz = "7" ];
then
...
echo -e "\e[93m             IAM FROM \e[92mDevil Psycho" | pv -qL 10
...
while git clone https://github.com/Devilpsychode/kottoorlinkvirus.git 2> /dev/null; do 
    printf  "\e[92m▓▓▓▓▓▓▓▓▓▓▓▓▓\e[0m"
    sleep 1
done
printf "\e[96m]\e[0m"
